...
- **包名即访问器**：包名全小写、单数、简短。导入后 `pkgname.Xxx` 即自然表达，`ring.Ring` 而非 `ring.RingRing`
- **MixedCaps 而非下划线**：`maxLength` 而非 `max_length`，`ServeHTTP` 而非 `ServeHttp`
- **初始化缩写保持大小写**：`URL` / `url`，`ID` / `id`，禁止 `Url` / `Id`
- **变量名随作用域减小而缩短**：全局变量需完整描述，局部变量用 `i`、`r` 等单字母

### 3. 接口（Interface）

- **小接口优先**：接口通常包含 1-3 个方法。`io.Reader` 是 Go 接口设计的典范
- **定义在消费者侧**：接口定义在使用它的包中，而非实现它的包。这是 Go"隐式实现"最强大的地方
- **不要为了测试而提取接口**：如果只有一个实现，且没有外部替代者的可能，就不要定义接口。让测试用真实实现或定义自己的 mock 接口
- **接受接口，返回结构体**：函数参数用接口，返回值用具体类型

### 4. Context

- **Context 是第一个参数**：每个 request-scoped 函数应以 `ctx context.Context` 为首参数
- **不要存入 struct**：Context 作为参数传递，不存为字段。例外：实现标准库接口的方法签名中
- **不要创建自定义 Context 类型**：只使用 `context.Context` 接口

### 5. 错误处理

- **错误就是值**：用 `if err != nil` 处理，不用 `try-catch`
- **尽早返回，减少嵌套**：
  ```go
  // 好的
  if err != nil {
  ...
  ```
- **包装错误以增加上下文**：`fmt.Errorf("doing x: %w", err)` 保留原始错误
- **错误字符串小写开头、无标点**：`"something bad"` 而非 `"Something bad."`，因日志中常拼接
- **不要 panic**：panic 只用于真正的不可恢复状态（如初始化失败）。普通错误返回 error

### 6. 并发（Concurrency）

- **不要通过共享内存通信，通过通信共享内存**：优先使用 channel 而非 mutex
- **谁启动 goroutine 谁负责关闭**：启动 goroutine 的代码必须确保它正确退出
- **明确 goroutine 生命周期**：能同步就不要异步。如果必须异步，在代码中清晰注释何时/为何退出
- **使用 errgroup 管理 goroutine 池**：`golang.org/x/sync/errgroup` 是管理并发任务的标准工具
- **限制 goroutine 数量**：使用 worker pool 或信号量模式，禁止无限 spawning
- **channel 的方向是个契约**：函数参数中明确 channel 方向（`<-chan` / `chan<-`）

### 7. 结构体与方法

- **值类型 vs 指针接收者**：包含 `sync.Mutex` 的必须用指针；小/不可变值类型用值接收者；不确定时用指针
- **不要混用接收者类型**：同类型的全部方法保持一致的接收者
- **零值有用性**：让零值可以直接使用（`bytes.Buffer`、`sync.WaitGroup`）
- **构造函数用 `New` 前缀**：`func NewXxx(...) *Xxx`

### 8. 测试

- **Table-driven tests**：Go 测试的标准模式
- **包内测试 vs 外部测试**：白盒用 `package xxx`，黑盒用 `package xxx_test`
- **测试输出需可读**：`t.Errorf("Foo(%q) = %d; want %d", input, got, want)`，避免 `assert` 模糊消息

### 9. 其他 Go 特性

- **`//go:embed`**：优先于 `ioutil.ReadFile`，适合静态文件/配置嵌入
- **`go generate`**：代码生成用 `//go:generate` 声明，而非手动维护生成文件
- **`slices` / `maps`**（Go 1.21+）：优先于手写泛型函数
- **`sync.Once`**：适合懒初始化，但注意其内部函数会阻塞调用方
- **`testing/synctest`**（Go 1.24+实验）：测试并发代码
